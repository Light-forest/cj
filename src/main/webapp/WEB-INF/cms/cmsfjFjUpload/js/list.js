$(document).ready(function() {
    ...
    inputFile.on('change', function( e ){
        ...
        //alert(name);
        dvd_set_val("fjsm",fileName);
    });
});

function  submitAddFj() {
         //<form id="formAdd" name="formAdd"
         //enctype="multipart/form-data"
         //action="addZp1.do?fuid=${cmsfj.fuid}" method="post">
         //<input type="hidden" name="_method" value="put">
        var action= $("#formAdd").attr("action");
        action+="&fjsm="+dvd_get_val("fjsm");
        $("#formAdd").attr("action",action);
        formAdd.submit();
}

function id_input_file_change() {
    var f = document.getElementById("id_input_file").files;
     //上次修改时间
...
function init_paixu_flag(){
	...
	var s1=orderbyfield.replace("desc","").trim();
    var s2=s1.replace("asc","").trim();
