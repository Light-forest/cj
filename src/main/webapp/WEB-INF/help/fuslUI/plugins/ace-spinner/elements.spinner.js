...
(function($ , undefined) {
	...
	function Ace_Spinner(element , options) {
		...
		var $element = $(element);
		$element.addClass('spinbox-input').wrap('<div class="ace-spinner">')
		...
		if(options.on_sides)
		{
			...
			.before('<div class="spinbox-buttons input-group-btn">\
					<button type="button" class="btn spinbox-down '+options.btn_down_class+'">\
						<i class="'+ ace.vars['icon'] + options.icon_down+'"></i>\
					</button>\
				</div>')
			.after('<div class="spinbox-buttons input-group-btn">\
					<button type="button" class="btn spinbox-up '+options.btn_up_class+'">\
