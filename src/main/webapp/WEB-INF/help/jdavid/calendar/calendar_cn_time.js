...
//日期+时间用法：
// 主调用函数是 setdaytime(this,[object])和setday(this)，[object]是控件输出的控件名，举两个例子：
// 一、<input name=txt><input type=button value=setdaytime onclick="setdaytime(this,document.all.txt)">
// 二、<input onfocus="setdaytime(this)">

var bMoveable = true;
var strFrame;

document
		.writeln('<iframe id=endDateLayer frameborder=0 width=162 height=211 style="position: absolute; z-index: 9998; display: none"></iframe>');
strFrame = '<style>';
...
strFrame += '         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="当前日期时间">当前</span>&nbsp;&nbsp;<span style="cursor:hand" id=evaAllOK onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" onclick="parent.closeLayer()" title="关闭日历">确定&nbsp;</span></td></tr>';
...
function setdaytime(tt, obj) // 主调函数
{
...
function tmpSelectYearInnerHTML(strYear) // 年份的下拉框
{
	...
	for ( var i = n; i < n + 141; i++) {
		if (i == m) {
			selectInnerHTML += "<option value='" + i + "' selected>" + i + "年"
					+ "</option>\r\n";
		} else {
...
function tmpSelectMonthInnerHTML(strMonth) // 月份的下拉框
{
	...
	for ( var i = 1; i < 13; i++) {
		if (i == m) {
			selectInnerHTML += "<option value='" + i + "' selected>" + i + "月"
					+ "</option>\r\n";
		} else {
