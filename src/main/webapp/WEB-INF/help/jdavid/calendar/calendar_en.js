...
document
		.writeln('<iframe id=endDateLayer frameborder=0 width=178 height=221 style="position: absolute; z-index: 9998; display: none"></iframe>');
strFrame = '<style>';
strFrame += 'INPUT.button{BORDER-RIGHT: #63A3E9 1px solid;BORDER-TOP: #63A3E9 1px solid;BORDER-LEFT: #63A3E9 1px solid;';
strFrame += 'BORDER-BOTTOM: #63A3E9 1px solid;BACKGROUND-COLOR: #63A3E9;font-family:Arial;}';
...
strFrame += '     <table border=0 cellspacing=0 cellpadding=2 bgcolor=#63A3E9 ' + (bMoveable ? 'onmousedown="DragStart()" onmouseup="DragEnd()"'
		: '');
strFrame += '    BORDERCOLORLIGHT=#63A3E9 BORDERCOLORDARK=#FFFFFF width=140 height=20 style="cursor:' + (bMoveable ? 'move'
		: 'default') + '">';
strFrame += '    <tr><td style="font-size:12px;color:#ffffff" width=20>&nbsp;Su</td>';
strFrame += '<td style="font-size:12px;color:#FFFFFF" >&nbsp;&nbsp;M</td><td style="font-size:12px;color:#FFFFFF">&nbsp;&nbsp;Tu</td>';
strFrame += '<td style="font-size:12px;color:#FFFFFF" >&nbsp;&nbsp;W</td><td style="font-size:12px;color:#FFFFFF" >&nbsp;&nbsp;Th</td>';
strFrame += '<td style="font-size:12px;color:#FFFFFF" >&nbsp;&nbsp;F</td><td style="font-size:12px;color:#FFFFFF" >&nbsp;&nbsp;Sa</td></tr>';
strFrame += '</table></td></tr>';
...
strFrame += '      <tr align=center >';
for (i = 35; i < 37; i++)
	strFrame += '<td width=20 height=20 id=meizzDay' + i + ' style="font-size:12px" onclick="parent.meizzDayClick(this.innerText,0)"></td>';
strFrame += '        <td colspan=5 align=right style="color:#1478eb"><span onclick="parent.setNull()" style="font-size:12px;cursor: hand"';
strFrame += '         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="将日期置空">clear</span>&nbsp;&nbsp;<span onclick="parent.meizzToday()" style="font-size:12px;cursor: hand"';
strFrame += '         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="当前日期时间">Current</span>&nbsp;&nbsp;<span style="cursor:hand" id=evaAllOK onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" onclick="parent.closeLayer()" title="关闭日历">OK&nbsp;</span></td></tr>';
...
document.onkeyup = ( function() {
	if (window.event.keyCode == 27) {
		...
		if (document.activeElement != outObject
...
function meizzWriteHead_(yy, mm, ss) // 往 head 中写入当前的年与月
...

{
	...
	odatelayer.meizzMonthHead.innerText = format(mm) + " Mon";
	// 插入当前小时、分
	odatelayer.meizzHourHead.innerText = bUseTime ? (meizzTheHour + " Hr") : "";
	odatelayer.meizzMinuteHead.innerText = bUseTime ? (meizzTheMinute + " Min")
			: "";
	odatelayer.meizzSecondHead.innerText = bUseTime ? (meizzTheSecond + " Sec")
			: "";
}
...
function meizzToday() // Today Button
{
	...
	if (outObject) {
		/****
		if (bUseTime) {
			outObject.value = parent.meizzTheYear + "-"
...
function meizzSetDay(yy, mm) // 主要的写程序**********
{
	...
	for (i = 0; i < 37; i++) {
		...
		if (meizzWDay[i] != "") {
			...
			{
				da.innerHTML = "<b><font color=#BCBABC>" + meizzWDay[i]
						+ "</font></b>";
				...
				if (!outDate)
					...
					da.style.backgroundColor = ((mm == 1 ? yy - 1 : yy) == outDate
							...
							.getDate()) ? "#84C1FF"
							: (((mm == 1 ? yy - 1 : yy) == new Date()
									.getFullYear()
									&& (mm == 1 ? 12 : mm - 1) == new Date()
...

{
	...
	WriteDateTo(meizzTheYear, meizzTheMonth, meizzTheDate, meizzTheHour,
...
//				+ format(hh) + ":" + format(mi) + ":" + format(se); // 注：在这里你可以输出改成你想要的格式
//	} else {
